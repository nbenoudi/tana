...
function Calculatrice() {
  ...
  const inputHandler = (event) => {
    if (answer === "ⵓⵀⵓ /Invalid Input!!") return;
  ...
  const calculateAns = () => {
    ...
    try {
      ...
      if (!checkBracketBalanced(finalexpression)) {
        const errorMessage = { message: " ⵓⵔ ⵙⵃⵉⵏ ⵍⴰⵇⵡⴰⵙ/Brackets are not balanced!" };
        throw errorMessage;
      }
      result = evaluate(finalexpression); //mathjs
    } catch (error) {
      result =
        error.message === "ⵓⵔ ⵙⵃⵉⵏ ⵍⴰⵇⵡⴰⵙ/Brackets are not balanced!"
          ? "Brackets are not balanced!"
          : "ⵓⵀⵓ /Invalid Input!!"; //error.message;
    }
